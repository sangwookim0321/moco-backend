  ...
  async (req, res) => {
    ...
    if (!testName || !testSubName || !testDescription) {
      return res.status(400).json({
        status: "error",
        message:
          "테스트 이름 또는 테스트 부제목 또는 테스트 설명을 입력해주세요.",
      })
    }
    ...
    if (!Array.isArray(types) || types.length < 2) {
      return res.status(400).json({
        status: "error",
        message:
          "타입(유형)을 최소 2개 이상 입력하거나, 적절한 타입 데이터를 제공해주세요.",
      })
    }
...
router.get("/getAdmins", checkAdminPermission, async (req, res) => {
  ...
  try {
    const { data, error } = await supabase
      .from("accounts")
      .select("id, role, username, phone, email")
      .or("role.eq.1, role.eq.2")
...
router.get("/statisticsList", checkAdminPermission, async (req, res) => {
  ...
  try {
    const { data, error } = await supabase
      .from("tests")
      .select("id, name, path, totalCount")
      .gt("totalCount", 0)
...
router.get("/statisticsDetail", checkAdminPermission, async (req, res) => {
  ...
  try {
    ...
    const { data: resultData, error: resultError } = await supabase
      .from("result")
      .select("id, test_id, type, description, count")
      .eq("test_id", testId)
